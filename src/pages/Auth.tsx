  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        if (error) throw error;
        toast.success("Account created! Welcome to RecipeVault!");
      }
    } catch (error: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md shadow-medium">
        <CardHeader className="space-y-3 text-center">
          ...
          <CardDescription>
            {isLogin
              ? "Sign in to access your recipes and ingredient lists"
              : "Create an account to start generating and saving recipes"}
          </CardDescription>
        </CardHeader>
        <CardContent>
